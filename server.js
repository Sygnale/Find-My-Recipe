...
app.get('/get-recipes/:userId', (req, res) => {
  ...
  con.query(tagQueryString, [userID], (err1,result1) =>{
    ...
    let recipeQueryStringEnd = `) AS T RIGHT JOIN recipe_ingredients ON T.ingredient_id=recipe_ingredients.ingredient_id
     GROUP BY recipe_id HAVING SUM(amount IS NULL OR amount < min_si)=0)`;
    ...
    con.query(recipeQueryStringStart+recipeQueryStringEnd, (err2,result2) => {
      if(err2) throw err2;
